...
@section('title', 'Online Order Details')
@section('cssLink')
    <script>
...
@section('backend')
    ...
    <section class="content-header mmm">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>Online Order(#{{ $order->id }}) Details</h1>
                </div>
                <div class="col-sm-6 d-flex justify-content-end">
                    ...
                    @else
                        <form action="{{ route('customer.shop.onlineOrderDelete') }}" method="post" class="mr-3">
                            @csrf
                            @method('delete')
                            <input type="hidden" name="id" value="{{ $order->id }}">
                            <button type="submit" onclick="return confirm('Are you sure want to delete this item?')"
                                class="btn btn-danger btn-sm">Cancel</button>
                        </form>
                        <form action="{{ route('customer.shop.onlineOrderStatus', $order->id) }}" method="post">
                            @csrf
                            <input type="hidden" name="status" value="{{ ++$order->status }}">
                            <button type="submit" class="btn btn-success btn-sm">{{ $order->order_status }}</button>
                        </form>
                    @endif
                </div>
            </div>
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="container">
                    <div class="card" id="print">
                        <div class="card-header">
                            ...
                            <span style="float: right;"> <strong>Status:</strong>
                                <span style="display: none">{{ --$order->status }}</span>
                                {{ $order->order_status }}
                            </span>
                    ...
                    <button id="print" onclick="printContent('print');" class="btn btn-success btn-sm">Print
                        Invoice</button>
                </div>
            </div>
            <!-- /.row -->
